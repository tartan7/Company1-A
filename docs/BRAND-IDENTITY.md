...
### Brand promise
UniteCube helps small and medium businesses run operations with confidence through clear, dependable workflow automation.
...
UniteCube is an operations automation platform for SMB teams that need enterprise-grade reliability without enterprise complexity.

## 2. Logo System

### Primary logo
- Preferred mark: `UniteCube` wordmark + rounded-square cube symbol.
- Primary application: navigation/header, docs cover, website hero, presentation title slides.
...
| --- | --- | --- |
| App navigation/sidebar | Full color on light background | Default for product UI |
| Marketing hero on dark photo/solid | Reverse on dark background | Ensure contrast behind full lockup |
| Favicon/app icon/compact badges | Symbol-only variant | Keep minimum size guidance |
| Single-ink exports/print constraints | Monochrome | Avoid gradients or effects |

### Clear space and minimum size
- Clear space: leave padding equal to at least the symbol corner radius on all sides.
...
- When exported brand assets are produced, store in `public/images/logo/` as SVG.

## 3. Color Palette

The palette below is canonical and aligns with current implementation in `public/css/style.css`.
...
- Interactive icon links/buttons use `color-brand-primary`, hover `color-brand-primary-dark`.
...
- Empty states: state condition + next action (`No workflow runs yet. Start your first workflow to see activity here.`).
...
- Destructive actions: semantic error color, with confirmation when irreversible.
...
### Change control
- Any change to colors, typography, or voice must update all three sources in the same PR when applicable.
...
- `2026-04-26`: Added `docs/BRAND-IDENTITY-REVIEW.html` as a visual review companion artifact.
- `2026-04-26`: Updated to `v1.1`; added logo variant usage matrix and governance-ready version marker.
